...
class WalletController extends Controller
{
    ...
    public function index()
    {
        ...
        $transaction = ModelsTransaction::where('user_id', Auth::id())->latest()->limit(50)->get();

        $account = UserAccount::where('user_id', Auth::id())->first();

        return view('pages.wallet')->with(['wallet'=> $wallet,
    ...
    public function sendPaymentRequest(Request $request){
        ...
        if($wallet = Wallet::where('user_id',  Auth::id())->first()) {
            if($wallet->amount < $request->amount) {
                ...
                $type = "Withdrawal";
                ModelsTransaction::create(['user_id' => Auth::id(),
